...
func (s *ClientTestSuite) TestClient_CreateUser() {
	...
	err := s.cl.CreateUser(context.Background(), &User{Email: "mktest@example.com"})
	...
	assert.Error(t, err)
	assert.True(t, IsErrInResponse(err))
...
func (s *ClientTestSuite) TestClient_UpdateUser() {
	...
	httpmock.RegisterResponder("PUT", "/security/users/id1", httpmock.NewStringResponder(400, "bad request"))
...
func (s *ClientTestSuite) TestClient_DeleteUser() {
	...
	httpmock.RegisterResponder("DELETE", "/security/users/id1", httpmock.NewStringResponder(200, ""))
	assert.NoError(t, s.cl.DeleteUser(context.Background(), "id1"))

	httpmock.RegisterResponder("DELETE", "/security/users/id2", httpmock.NewStringResponder(500, ""))
...
func (s *ClientTestSuite) TestClient_GetUser() {
	...
	_, err = s.cl.GetUser(context.Background(), "wrong_email")
	assert.Error(t, err)
