...
var _ = Describe("NexusUser controller", func() {
	...
	It("should fail if secret not found", func() {
		By("creating a new NexusUser object")
		newNexusUser := &nexusApi.NexusUser{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "nexus-user-invalid",
				Namespace: namespace,
			},
			Spec: nexusApi.NexusUserSpec{
				ID:        "nexus-user-invalid",
				FirstName: "user-first-name",
				LastName:  "user-last-name",
				Email:     "user-email@gmail.com",
		...
		Eventually(func(g Gomega) {
			createdNexusUser := &nexusApi.NexusUser{}
			err := k8sClient.Get(ctx, types.NamespacedName{Name: "nexus-user-invalid", Namespace: namespace}, createdNexusUser)
			g.Expect(err).ShouldNot(HaveOccurred())
			g.Expect(createdNexusUser.Status.Value).Should(Equal(common.StatusError))
			g.Expect(createdNexusUser.Status.Error).ShouldNot(BeEmpty())
